...
```
/pr-review <pr-number-or-url> [--post]
```
...
**不适用**:纯本地 diff 的 review(用 `/code-review`);issue 里的设计评审(用 `/issue-review`)。
...
echo "${hdr} — Claude Code(<model>)。读取:PR diff + 受影响代码/文档/测试 + 关联 issue。运行:/verification (pre-merge) + <测试命令>。每条结论附可复现证据。"
```
