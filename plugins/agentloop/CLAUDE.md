...
`deployment.json`+`repos.json` 并装 crontab（本地）/ RemoteTrigger routine（云端）。它是 `bootstrap`
（把单个 repo 变成可消费：profile+labels+verify）的**调度对侧**——bootstrap 故意不碰调度。规矩：
...
- `lib/*.test.ts`（report / comment / gate 引擎）+ `skills/issue-sweep/test/sweep-golden/`
  （marker / round-awareness 金测）。改引擎或 marker 逻辑**先看 golden**。
