...
| `skillEnv` | | — | per-skill env keyed by skill local name; `{{CHECKOUT}}` expands. Optional. Daemon isolation is **not** done here — skills that boot `arc service` start a named instance (`arc service start fleet-issue-sweep` / `fleet-pr-sweep`, then `arc service url NAME`). Hand-tuned values are preserved across re-runs |
...
| `skillConcurrency` | | per-skill active-worker limit for this repo, e.g. `{ "issue-sweep": 3 }`; integer `1..16`. The driver injects it as `AGENTLOOP_SKILL_CONCURRENCY`; each skill defines worker semantics and its fallback default |
