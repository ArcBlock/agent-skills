 ...
 *
 *  `launcher` and `currentLink` live in the user's OWN config dir — never touched by a
 *  plugin install/upgrade — which is exactly why the crontab references them instead of
 *  a path inside the (version-numbered, upgrade-deleted) plugin cache. See
 *  renderRunnerScript for what `launcher` actually does. */
export function paths(input: Pick<SetupInput, "configDir" | "logDir">) {
  ...
  return {
    ...
    launcher: j(dir, "agentloop-run.ts"),
    currentLink: j(dir, "agentloop-current"),
  };
}
...
/**
 * Content of the STABLE launcher script the crontab actually invokes (never a versioned
 * plugin path directly — see paths().launcher).
 *
 * WHY THIS EXISTS: a plugin version bump DELETES the old version's directory. A crontab
 * row that hardcodes `bun <old-version>/fleet/driver.ts` then fails on every single tick
 * with a bare "Module not found" — Bun can't even load the entry file, so nothing of
 * driver.ts's own error handling ever runs. That failure is invisible: it only shows up in
 * a redirected log file nobody tails, not in any comment/PR/alert. MEASURED live: 4+ hours
 * of total silence on a real deployment before a human happened to ask why.
 *
 * FIX: the launcher re-resolves "where does agentloop actually live right now" on EVERY
 * invocation and execs into whatever it finds — so the very next scheduled tick after an
 * upgrade picks up the new version automatically, with no human action required.
 *
 * MUST be fully self-contained (no imports from the plugin itself, no template-literal
 * nesting with backticks): resolving where a possibly-just-deleted version lives is
 * exactly the job, so it cannot depend on that version already being reachable. Written
 * with plain string concatenation (not backticks) inside the generated code on purpose —
 * nesting backtick template literals here is a real footgun (the outer literal in THIS
 * file would need every inner backtick/`${` escaped, and a missed one fails silently at
 * generation time, not at the call site that would catch it).
 */
export function renderRunnerScript(bunPath: string, currentLinkPath: string): string {
  const lines = [
    "#!/usr/bin/env bun",
    "// GENERATED by fleet/setup.ts (renderRunnerScript) — hand edits are lost on the next",
    "// `fleet/setup.ts --apply --local` reconcile. See that function's doc comment for why",
    "// this file exists: it is the STABLE thing crontab invokes so a plugin version bump",
    "// (which deletes the old version's directory) can never silently break every tick.",
    ...
    "}",
    "",
    "function scanCache(cacheBase) {",
    "  const found = [];",
    "  if (!existsSync(cacheBase)) return found;",
    "  for (const mkt of readdirSync(cacheBase)) {",
    '    const pdir = cacheBase + "/" + mkt + "/agentloop";',
    "    if (!existsSync(pdir)) continue;",
    "    for (const ver of readdirSync(pdir)) {",
    '      const full = pdir + "/" + ver;',
    '      if (existsSync(full + "/.claude-plugin/plugin.json")) found.push({ dir: full, ver });',
    "    }",
    "  }",
    "  return found;",
    "}",
    "",
    "function highestOf(found) {",
    "  if (!found.length) return undefined;",
    "  found.sort((a, b) => cmpSemver(b.ver, a.ver));",
    "  return found[0].dir;",
    "}",
    "",
    "function findRoot() {",
    "  const home = homedir();",
    "  // codex has NO per-invocation --plugin-dir: it loads the skill it invokes BY NAME",
    "  // from wherever it is GLOBALLY installed, independent of what we pass via --add-dir.",
    "  // So whenever codex has ANY install, that install MUST win — pointing elsewhere (even",
    "  // at an equally-current marketplace clone) risks the prompt (read from whatever root",
    "  // we resolve) and the skill codex actually loads (read from codex's own registry)",
    "  // landing on two different versions. See detectCodexPluginRoot's doc comment.",
    '  const codex = highestOf(scanCache(home + "/.codex/plugins/cache"));',
    "  if (codex) return codex;",
    "  // No codex install at all: claude's own preference is the marketplace clone — kept",
    "  // current via `git pull` in place, so a plugin-manager delete+install can never stale it.",
    ...
    '  return highestOf(scanCache(home + "/.claude/plugins/cache"));',
    "}",
    "",
    "const root = findRoot();",
    "if (!root) {",
    "  console.error(",
    '    "\\u2717 agentloop-run: no agentloop plugin installation found " +',
    '      "(checked the claude marketplace clone, the codex plugin cache, and the claude plugin cache).\\n" +',
    '      "  Reinstall it, e.g.:\\n" +',
    ...
    "}",
    "",
    "// Best-effort: keep a stable symlink pointing at whatever we just resolved, so",
    "// deployment.json's agentloopRoot (used for --plugin-dir/--add-dir when the driver",
    "// spawns claude/codex) never goes stale either — it points HERE, not at a literal",
    "// version. A failed relink must never block the actual run.",
    `const link = ${JSON.stringify(currentLinkPath)};`,
    "try {",
    "  try {",
    "    unlinkSync(link);",
    "  } catch {}",
    ...
    "} catch {}",
    "",
    `const proc = Bun.spawnSync([${JSON.stringify(bunPath)}, root + "/fleet/driver.ts", ...process.argv.slice(2)], {`,
    '  stdout: "inherit",',
    '  stderr: "inherit",',
    '  stdin: "inherit",',
    "});",
...
export function renderCronRow(input: SetupInput, skill: string, minute: number): string {
  const p = paths(input);
  // The STABLE launcher, never a versioned plugin path — see renderRunnerScript's doc
  // comment for why: a hardcoded `<agentloopRoot>/fleet/driver.ts` here would go stale
  // and fail silently on every tick the moment agentloopRoot's version gets deleted by
  // the next plugin upgrade.
  const run = `${input.bunPath} ${p.launcher} --config ${p.deployment} --catalog ${p.catalog} --skill ${skill} --run`;
...
if (import.meta.main) {
  ...
  // LOCAL only: point agentloopRoot at the stable, launcher-maintained symlink instead of
  // the raw resolved (version-numbered) path — see renderRunnerScript. The cloud plan
  // below still uses the raw `agentloopRoot` (a cloud routine runs in its own remote
  // sandbox, where this local symlink would not even exist).
  if (local) deployment.agentloopRoot = p.currentLink;
  ...
  if (!apply) {
    ...
    if (local) {
      ...
      console.log(`\n# (would also write the stable launcher: ${p.launcher})`);
    } else {
  ...
  if (local) {
    // The stable launcher crontab actually invokes — regenerated every reconcile so it
    // always reflects the CURRENT bunPath/currentLink, same idempotent contract as the
    // crontab block and the config files. See renderRunnerScript's doc comment.
    writeFileSync(p.launcher, renderRunnerScript(input.bunPath, p.currentLink), { mode: 0o755 });
    console.log(`✓ wrote ${p.launcher}`);
