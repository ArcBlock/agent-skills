...
# Dirty tree → stash (recoverable), never discard: it may be a concurrent
# session's uncommitted work (a blind hard-reset here has previously destroyed
# in-flight edits from another session — arc case-law).
[ -n "$(git status --porcelain)" ] && git stash push -u -m "issue-sweep preempted $(date -u +%FT%TZ)"
git reset --hard origin/<default_branch>   # tree is clean now; this only moves the ref
...
- An **AI/agent comment contains** `🤖 AI Agent` near the top (audit or action
  note) — **do not require it to be the literal first line.** CLAUDE.md's
  `Agent Comment 格式` convention puts a `@ <hostname>` identity line *before*
  it (`@ vm\n\n> 🤖 AI Agent — ...`), and older comments used bare `@ <hostname>`
  or a differently-worded `> 🤖 ...` opener. A strict `startswith('> 🤖 AI
  Agent')` check misses all of these and reclassifies already-answered issues
  as fresh human input — a real regression that wasted a full sweep re-reading
  issues that had already reached a terminal needs-human-confirm state (found
  2026-07-11). Treat a comment as agent-authored if it contains `🤖 AI Agent`
  (or a `🤖` blockquote opener) in its first ~3 lines, **or** it ends with the
  `_Generated by [Claude Code]` footer, **or** it carries a `<!-- sweep-trace:
  ... -->` marker.
- A **human comment** is anything else.
...
whole class of doable issues never advanced. Cheap detection: last `🤖 AI Agent` comment
matches `排队|本轮未做|留开放|不在本轮|不在.{0,8}范围|非单 PR|多.?phase.*(留开放|不在)|candidate-queued|queued|in-progress|稍后|will do|TODO` → re-process.
...
| **父级 rollup**（Step 0.5 `rollupCandidates`：open 父 issue 的孩子已全部关闭） | 走 [`issue-review` ★父级 rollup](../issue-review/SKILL.md)：`claim.ts` fencing 抢到才做 → 核对父 issue 验收标准/问题清单（逐条对应到子 issue/PR 证据）→ 综合 comment（带 rollup marker）→ **全覆盖则 close**（这是「自动 close」的显式例外），有残留 gap 列出并留开。research/idea 类父 issue 同样综合后 close——结论已在子 issue 落地，父级只是收口。 |
