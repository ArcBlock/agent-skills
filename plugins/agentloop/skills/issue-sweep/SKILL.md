...
- An **AI/agent comment contains** `🤖 AI Agent` near the top (audit or action
  ...
  ... -->` marker, **or** it carries the machine identity line
  (`runner:… · skills@…`), **or** it opens with another script/bot blockquote
  marker — notably `> 📡` (the presence-board heartbeat).
- **`🤖` alone is NOT enough** (live-sweep finding): the presence board's `> 📡`
  heartbeat is written *by a script under a human account* and re-edited every
  round, so a 🤖-only predicate reads it as **fresh human input every single
  round** and re-processes the board forever. Same class: markers can arrive
  HTML-escaped (`&lt;!-- … --&gt;`, the #1278 double-escape bug) — **decode
  entities before matching any marker**, or marker-keyed logic silently misses them.
