...
_Generated by [Claude Code](https://claude.ai/code)_`;
  ...
  // arc#1104: last verdict comment said "## Rollup 复核" (not "核对") — the narrower
  // regex missed it, so graph-scan re-surfaced an already-settled partial-rollup issue.
  test("verdict comment uses '复核' instead of '核对' wording → still recognized as reviewed (#1104)", () => {
    expect(
      isRollupAlreadyReviewed(
        "> 🤖 AI Agent Audit\n\n## Rollup 复核：最后一条尾巴已随 PR #1872 合并落地\n\nagent:hold 仍在，本条只做综合确认，不 close。",
        "2026-07-20T04:13:28Z",
        "2026-07-18T06:20:00Z",
      ),
    ).toBe(true);
  });

  // arc#114: the close-kick path posted "## Kick 复核" (neither "父级 rollup" nor
  // "Rollup 核对/复核") — a hardcoded per-wording regex would need yet another
  // literal entry; the proximity match (rollup|kick|父级 near 核对|复核) covers it
  // without another edit.
  test("kick-triggered verdict comment ('Kick 复核') → still recognized as reviewed (#114)", () => {
    expect(
      isRollupAlreadyReviewed(
        "> 🤖 AI Agent Audit\n\n## Kick 复核：4 个子 issue 已全部 closed，但全链路目标未达成，`agent:hold` 不动\n\n结论：4 个直接子 issue 全闭不等于本 issue 的验收目标达成。",
        "2026-07-20T21:17:28Z",
        "2026-07-20T20:15:00Z",
      ),
    ).toBe(true);
  });

  // arc#753 / arc#3715: verdict comment uses "核实" (neither "核对" nor "复核") — the
  // narrower two-synonym regex missed it, so graph-scan re-surfaced an already-settled
  // rollup (7/7 children closed, agent:hold vetoing close, verdict already posted).
  test("verdict comment uses '核实' instead of '核对'/'复核' wording → still recognized as reviewed (#753/#3715)", () => {
    expect(
      isRollupAlreadyReviewed(
        "> 🤖 AI Agent Audit\n\n## 父级 rollup 核实：7/7 子任务全部关闭，验收标准已满足",
        "2026-08-10T05:20:00Z",
        "2026-08-10T05:02:03Z",
      ),
    ).toBe(true);
  });
});
