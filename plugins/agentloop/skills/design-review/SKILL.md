...
每条本 skill 发往 issue 的 设计评审 comment 末尾**必须**附一行 sweep-trace HTML 注释（人不可见、grep 可查）：

```html
<!-- sweep-trace: {"ver":1,"issue":N,"gate":"design","val":"<val>","run":"<ISO8601>","runner":"<runner>","skills":"<hash>"} -->
```
