...
JQ='
  ...
  def isCodex: (.login // "") | test("chatgpt-codex-connector|codex-connector";"i");
...
- **★ Review 闸(与 verification 闸同级,独立于 `mergeable_state` 的判读)**:
  ...
  另一处仍能拦住,不互相依赖。且
- **★ Codex / bot-review short-wait(推进优先,不长时间挂起):** 本 org 的 Codex 在 open/ready/push
  后通常**数分钟内**给 inline findings,或只留一个 **👍** 表示无意见。规则:
  - **👍 / 无 inline finding** → 不挡 merge(不算人类 review 闸)。
  - **有 P1 inline finding 且 agent 尚未 fix 或 REJECT-reply** → 当可执行活:打/保持
    `pr-sweep:needs-fix`,在 PR 分支修或 thread 回绝,再重跑 `pre-merge`;**不要**因"等 Codex
    再评一次"挂数小时。
